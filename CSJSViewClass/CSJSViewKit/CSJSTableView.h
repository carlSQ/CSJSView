...
@protocol TableViewJSExport<JSExport>

+ (instancetype)jsTableViewWithFrame:(CGRect)frame andStyle:(UITableViewStyle)style;

@property (nonatomic, strong) JSValue *jsDelegate;
...
- (__kindof UITableViewCell *)dequeueReusableCellWithIdentifier:(NSString *)identifier forIndexPath:(NSIndexPath *)indexPath ; // newer dequeue method guarantees a cell is returned and resized properly, assuming identifier is registered
...
@interface CSJSTableView :UITableView <TableViewJSExport,UITableViewDelegate, UITableViewDataSource>
